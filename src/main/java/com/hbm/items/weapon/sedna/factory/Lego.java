...
public class Lego {
    ...
    public static BiConsumer<ItemStack, LambdaContext> LAMBDA_STANDARD_RELOAD = (stack, ctx) -> {
        ...
        if(state == GunState.IDLE) {
            ...
            if(mag.canReload(stack, ctx.inventory)) {
                ...
                ItemGunBaseNT.playAnimation(player, stack, AnimationEnums.GunAnimation.INSPECT, ctx.configIndex);
    ...
    public static BiConsumer<ItemStack, LambdaContext> LAMBDA_STANDARD_FIRE = (stack, ctx) -> doStandardFire(stack, ctx, AnimationEnums.GunAnimation.CYCLE, 0, true);
    public static BiConsumer<ItemStack, LambdaContext> LAMBDA_SECOND_FIRE = (stack, ctx) -> doStandardFire(stack, ctx, AnimationEnums.GunAnimation.CYCLE, 1, true);
    /** Spawns an EntityBulletBaseMK4 with the loaded bulletcfg, ignores wear */
    public static BiConsumer<ItemStack, LambdaContext> LAMBDA_NOWEAR_FIRE = (stack, ctx) -> doStandardFire(stack, ctx, AnimationEnums.GunAnimation.CYCLE, 0, false);
    ...
    public static BiConsumer<ItemStack, LambdaContext> LAMBDA_LOCKON_FIRE = (stack, ctx) -> {
        doStandardFire(stack, ctx, AnimationEnums.GunAnimation.CYCLE, 0, true);
        ItemGunBaseNT.setIsLockedOn(stack, false);
    };

    public static void doStandardFire(ItemStack stack, LambdaContext ctx, AnimationEnums.GunAnimation anim, int receiver, boolean calcWear) {
        EntityLivingBase entity = ctx.entity;
        EntityPlayer player = ctx.getPlayer();
        int index = ctx.configIndex;
    ...
    @SuppressWarnings("incomplete-switch") public static BiFunction<ItemStack, AnimationEnums.GunAnimation, BusAnimationSedna> LAMBDA_DEBUG_ANIMS = (stack, type) -> switch (type) {
