...
public class VaultDungeonRoomSleep extends VaultDungeonRoomElevator {
    ...
    public void generateRoom(VaultDungeonPlacer placer, int x, int y, int z) {
        placer.generateBox(x + getW() / 2 - 2, y + 2, z + 2, 1, getH() - 4, getW() - 4, parent.wall);
        placer.generateBox(x + getW() / 2 - 2, y + 3, z + 2, 1, 1, getW() - 4, getLine(x, z));

        placer.generateBox(x + getW() / 2 + 2, y + 2, z + 2, 1, getH() - 4, getW() - 4, parent.wall);
        placer.generateBox(x + getW() / 2 + 2, y + 3, z + 2, 1, 1, getW() - 4, getLine(x, z));

        placer.generateBox(x + getW() / 2 - 2, y + 2, z + getW() / 2 - 1, 5, 3, 3, air);
    }
    ...
    public void placeLoot(VaultDungeonPlacer placer, int x, int y, int z) {
        ...
        placer.placeLootChest(new BlockPos(x + 4, y + 2, z + 4), EnumFacing.EAST, lockerLoot, 8);
        placer.placeLootChest(new BlockPos(x + getW() - 5, y + 2, z + getW() - 5), EnumFacing.WEST, lockerLoot, 8);
        placer.placeLootChest(new BlockPos(x + getW() / 2, y + 2, z + getW() / 2), EnumFacing.NORTH, lockerLoot, 7);
        placer.placeLootChest(new BlockPos(x + 4, y + 2, z + getW() - 5), EnumFacing.SOUTH,
